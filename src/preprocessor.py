...
class Preprocessor:
    ...
    def process(self, image):
        ...
        image_threshold = cv2.adaptiveThreshold(image_blur, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 19, 2)

        kernel = np.ones((3, 3), dtype = np.uint8)
        image_opening = cv2.morphologyEx(image_threshold, cv2.MORPH_OPEN, kernel, iterations = 2)
        ...
        ret, image_foreground = cv2.threshold(image_distance, 0.5 * image_distance.max(), 255, 0)
        image_foreground = np.uint8(image_foreground)

        image_unknown = cv2.subtract(image_background, image_foreground)

        num_markers, markers, stats, centroids = cv2.connectedComponentsWithStats(image_foreground, 4, cv2.CV_32S)
        markers = markers + 1
        ...
        red_mask = cv2.inRange(image_watershed, np.array([0, 0, 255]), np.array([0, 0, 255]))
        image_watershed = cv2.bitwise_and(image_watershed, image_watershed, mask = red_mask)
        image_watershed = cv2.cvtColor(image_watershed, cv2.COLOR_BGR2GRAY)
        
        ret, image_watershed_threshold = cv2.threshold(image_watershed, 0, 255, cv2.THRESH_BINARY_INV)

        contours, hierarchy = cv2.findContours(image_watershed_threshold, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)    
        image_contours = np.zeros(image.shape, dtype = np.uint8)
        cv2.drawContours(image_contours, sorted(contours, key = cv2.contourArea)[:-1], -1, 255, -1)
